...
// Parse a QR payload. Two canonical formats:
//
//   1. http(s)://<server>?token=<token>   ← preferred (works in any browser
//                                           too — TokenManager auto-signs in)
...
function parsePayload(raw: string): { server?: string; token?: string; title?: string } | null {
  const s = raw.trim();
  if (!s) return null;

  // Plain http(s) URL — only treated as a team-add when the URL carries
  // ?flag=addTeam (the marker cicy-code's web QR generator emits). This way
  // arbitrary http URLs scanned by accident don't try to "log in" the user.
  if (/^https?:\/\//i.test(s)) {
    try {
      const u = new URL(s);
